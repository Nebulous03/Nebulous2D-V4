...
public class Input {
    ...
    public static void update() {
        ...
        for (int i = 0; i < MOUSE_CODES; i++)
            if (!isMouseButtonHeld(windowID, i) && CURRENT_MOUSE.contains(i))
                UP_MOUSE.add(i);
        ...
        for (int i = 0; i < MOUSE_CODES; i++)
            if (isMouseButtonHeld(windowID, i))
                CURRENT_MOUSE.add(i);
 
        UP_KEYS.clear();
        for (int i = 0; i < KEY_CODES; i++)
            if (!isKeyHeld(windowID, i) && CURRENT_KEYS.contains(i))
                UP_KEYS.add(i);
    ...
    public static void setMousePosition(float f, float g) {
        GLFW.glfwSetCursorPos(windowID, f, g);
    }
